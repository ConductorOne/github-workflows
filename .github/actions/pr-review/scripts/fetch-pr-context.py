...
# Markers, heading trust boundaries, and working/completed comment selection
# are shared with publish-review-report.py so both sides of the publication
# contract classify comments identically.
from _review_state import (
    BOT_LOGINS,
    DEFAULT_REVIEW_SUMMARY_HEADING,
    LEGACY_REVIEW_SUMMARY_HEADING,
    PROVISIONAL_MARKER,
    is_valid_summary_heading,
    matching_heading,
    select_working_slot_and_state,
)
...
def review_comment_heading(comment: dict, summary_heading: str) -> Optional[str]:
    return matching_heading(comment["body"], summary_heading)
...
def extract_review_state(
    ...
    """Choose the working summary comment to update and the authoritative
    completed review state.

    Returns (summary_comment_id, last_reviewed_sha, last_review_base_sha).
    Comment identity and completed-review state are selected independently:

    - summary_comment_id is the newest WORKING comment (provisional or
      markerless): an earlier in-progress run's unfinished output, which a
      retried run updates instead of posting a duplicate next to it.
      Completed reports are NEVER returned here — the model must not mutate
      a completed report.
    - last_reviewed_sha/last_review_base_sha come from the newest COMPLETED
      report (owned, non-provisional marker). A provisional marker never
      supplies state: it is in-progress output and must not advance reviewed
      state. When no completed state exists the caller falls back to full
      review mode but still updates the same working comment.
    ...
    marker fails to parse fails closed the same way, and a superseded
    (collapsed) comment is archived output, never a candidate. Markerless bot
    summaries remain reusable working slots under the heading/bot trust
    fallback (the caller passes only bot-authored comments matching the
    selected heading), but they carry no state. Callers pass only comments
    matching the selected heading (legacy-heading comments included solely
    for the built-in production headings), so a custom heading can never
    adopt production or legacy review state.
    """
    working, _, state = select_working_slot_and_state(review_comments, workflow_ref)
    summary_comment_id = working["id"] if working else None
    last_reviewed_sha = (state or {}).get("last_reviewed_sha")
    last_review_base_sha = (state or {}).get("base_sha")
