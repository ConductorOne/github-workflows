...
- Fetching is restricted to a per-repository domain allowlist configured by the action's
  `doc_fetch_domains` input. Most repositories have not opted in, so `WebFetch` is usually
  unavailable. If the tool is unavailable, or a URL is outside the allowlist and the fetch is
  denied, that is an E4 outcome: skip the fetch, still apply C9, C11, and B10 from the diff
  alone, and state in the summary that vendor documentation was not verified. Never treat a
  denied fetch as evidence the endpoint is current.
